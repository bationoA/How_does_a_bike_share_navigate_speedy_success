...
df %>% filter(ride_length >= lower_bound & ride_length <= upper_bound) %>% 
  ...
  labs(fill = "Customers", 
       ...
       caption = "Trips initiated by casual customers tend to take much more time than the ones initiated by customers with annual subscription.",
  ) + 
...
# Save the plot as png image
file_name = file.path('outputs', 'img', 'bar_chart__average_number_of_rides_and_duration_per_customer_by_year.png')
## ggsave save the last image produced
ggsave(file_name,
       combined_plot,
       width=img_width,
